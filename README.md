...
1. Есть возможность реализовать дополнительный функционал для очистки списка предыдущих времен запросов cleanupRequestTimes().
2. Есть возможность реализовать дополнительный функционал для обработка ответа от API и сохранение созданного документа в базе данных или другом хранилище performApiCall().
Для доп функционала потребуется создать lock и unlock блоки.

В данном коде объект класса  Document document = crptApi.new Document(); выступает как модель которую мы используем для взаимодействия с CRPT API. 
